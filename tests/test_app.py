...
def test_parser_extracts_dependencies_and_parameters():
    spec = deterministic_parse(
        "S3 bucket and Lambda triggered on upload with 512 MB and timeout 45 in us-west-2",
        "demo-stack",
    )
    assert spec.s3_bucket and spec.lambda_function and spec.s3_trigger and spec.iam_role
    assert (
        spec.lambda_memory_mb == 512
        and spec.lambda_timeout_seconds == 45
        and spec.region == "us-west-2"
    )
...
def test_project_contains_secure_modular_resources():
    ...
    assert (
        "aws_s3_bucket_public_access_block" in files["main.tf"]
        and "server_side_encryption" in files["main.tf"]
    )
    assert security_scan(files) == []


def test_security_scan_flags_unsafe_configuration():
    assert security_scan({"main.tf": 'resource "aws_s3_bucket" "x" { acl="public-read" }'})


def test_structural_validator_rejects_broken_project(monkeypatch):
    monkeypatch.setattr(shutil, "which", lambda _: None)
    assert not validate_project({"main.tf": 'resource "aws_s3_bucket" "x" {'}, False).valid
...
def test_api_returns_zip_and_files(monkeypatch):
    ...
    with TestClient(app) as client:
        r = client.post(
            "/generate",
            json={
                "request": "create encrypted S3 bucket and DynamoDB",
        ...
        body = r.json()
        assert body["validation"]["valid"] and body["bundle_base64"] and "main.tf" in body["files"]
